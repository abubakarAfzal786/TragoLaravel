...
<form class="form form-vertical" method="POST" action="{{ route('transaction.update' , $trans->id ) }}">
@csrf
@method('PUT')
    ...
    <select data-select-2="" name="transtype"
        ...
        @if(!empty($types))
            @foreach($types as $key => $list)
                @if($list->id == $trans->transactionTypeId)
                <option value="{{$list->id}}" selected="">{{$list->description}}</option>
             @else
                <option value="{{$list->id}}">{{$list->description}}</option>
             @endif
    ...
    <select data-select-2="" name="address"
        ...
        @if(!empty($addresses))
            @foreach($addresses as $key => $list)
                @if($list->id == $trans->placeId)
                <option value="{{$list->id}}" selected="">{{$list->addressDescription.' '.$list->addressAddress}}</option>
             @else
...
<div class="col-sm-8" id="wrap-indirizzo_carico">
    <input name="time" type="date" class="form-control input-sm" value="{{$trans->time}}"
...
<div class="col-sm-8" id="wrap-descrizione_cdc_carico">
    <input name="docId" type="text" class="form-control input-sm"
           value="{{$trans->docId}}" placeholder="Descrizione CDC Carico">
</div>
</div>
...
<div class="col-sm-8" id="wrap-descrizione_cdc_carico">
    <input name="ddtId" type="text" class="form-control input-sm"
           value="{{$trans->ddtId}}" placeholder="Descrizione CDC Carico">
</div>
</div>
